...
def getdata_2d(dof=5, samp=10000, mean=[0.,1.], final_cov=[1., 2.], cov=[[1.,-0.8], [-0.8,1.]]):
 
    mvnorm=st.multivariate_normal(mean=mean, cov=cov)
    x=mvnorm.rvs(samp, random_state=7)
    norm=st.norm()
    x_unif=norm.cdf(x)
    m0=st.norm(loc=mean[0], scale=final_cov[0])
...
if __name__ == '__main__':
    ...
    from vae_tf1 import CVAE
    CVAE=CVAE(n_latent=2, n_hidden=100,alpha=0.5)
    #CVAE.train(res_norm,n_epochs=1000, learning_rate=0.0001, show_progress=True)
    
    

    CVAE.train(data=data_scaled, n_epochs=1000, learning_rate=0.0001, show_progress=True)
